...
def test_docid_hash(tmpdir):
    """Verifies that docids are computed correctly in map1.py.
    ...
    with tmpdir.as_cwd():
        madoop.mapreduce(
            input_path=TESTDATA_DIR/"test_pipeline16/crawl",
            output_dir=tmpdir/"output",
            map_exe="./map1.py",
            reduce_exe="./reduce1.py",
            num_reducers=4,
            partitioner=None,
        )

    # Verify docid hashes from job 1
    output_dir_path = Path(tmpdir/"output")
    output_file_paths = output_dir_path.glob("part-*")
    output_files = [path.read_text(encoding='utf-8')
                    for path in output_file_paths]
    output = '\n'.join(output_files)

    # Correct docids should exist in the output file
    assert "739261" in output
    assert "4436465" in output
    assert "9229752" in output
